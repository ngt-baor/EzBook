...
<div class="auth-shell">
    <h1 class="auth-title">Khách Hàng</h1>
    <p class="auth-subtitle">Đăng nhập để đặt lịch online</p>

    <c:if test="${param.msg == 'register-success'}">
        <p class="alert success">Đăng ký thành công. Vui lòng đăng nhập.</p>
    </c:if>
    <c:if test="${param.msg == 'logged-out'}">
        <p class="alert success">Bạn đã đăng xuất.</p>
    </c:if>
    <c:if test="${param.error == 'login-required'}">
        <p class="alert error">Bạn cần đăng nhập để đặt lịch online.</p>
    </c:if>
    <c:if test="${param.error == 'invalid'}">
        <p class="alert error">Tài khoản hoặc mật khẩu không đúng.</p>
    </c:if>
    <c:if test="${param.error == 'blocked'}">
        <p class="alert error">Tài khoản đang bị khóa.</p>
    </c:if>
    <c:if test="${param.error == 'role-not-allowed'}">
        <p class="alert error">Tài khoản này không thuộc nhóm khách hàng.</p>
    </c:if>
    <c:if test="${param.error == 'forbidden'}">
        <p class="alert error">Bạn không có quyền truy cập trang này.</p>
    </c:if>
    ...
    <div class="auth-links">
        <a href="${pageContext.request.contextPath}/khach-hang/dang-ky.jsp">Đăng ký ngay</a>
